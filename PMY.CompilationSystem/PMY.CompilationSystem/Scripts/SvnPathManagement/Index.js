...
var ButtonInit = function () {
    ...
    oInit.Init = function () {
        $("#btn_addsvnfolder").click(function () {
            $("#svnfolderModalLabel").text("新增");
            $("#txt_Id").val("");
            $("#txt_FolderName").val("");
            $("#txt_FolderPath").val("");
            $("#txt_Sort").val("");
            postdata.Id = "";
            $('#svnfolderModal').modal()
        });

        $("#btn_editsvnfolder").click(function () {
            ...
            $("#svnfolderModalLabel").text("编辑");
            $("#txt_Id").val(arrselections[0].Id);
            $("#txt_FolderName").val(arrselections[0].FolderName);
            $("#txt_FolderPath").val(arrselections[0].FolderPath);
        ...
        $("#btn_deletesvnfolder").click(function () {
            var arrselections = $("#tb_svnfolders").bootstrapTable('getSelections', function (row) { return row; });
            if (arrselections.length <= 0) {
                toastr.warning('请选择有效数据');
                return;
            ...
            Ewin.confirm({ message: "确认要删除选择的数据吗？" }).on(function (e) {
                ...
                $.ajax({
                    type: "post",
                    url: "/SvnPathManagement/Delete",
                    dataType: 'json',//服务端返回数据的格式，如果不对会跳到error
                    data: { svnFolderList:arrselections },
        ...
        $("#btn_selectfolder").click(function () {
            var folderPath = selectFolder();
            $("#txt_FolderPath").val(folderPath);
        })

        $("#btn_submitsvnfolder").click(function () {
            postdata.Id = $("#txt_Id").val();
            postdata.FolderName = $("#txt_FolderName").val();
            postdata.FolderPath = $("#txt_FolderPath").val();
            postdata.Sort = $("#txt_Sort").val();
            $.ajax({
                type: "post",
                url: "/SvnPathManagement/AddOrEdit",
                dataType: 'json',
                data: { svnFolders: postdata },
                success: function (data, status) {
                    if (status == "success" && data.Result == 1) {
                        toastr.success('提交数据成功');
                        $("#tb_svnfolders").bootstrapTable('refresh');
